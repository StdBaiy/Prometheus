...
if __name__ == "__main__":
    ...
    if not os.path.exists(PLUGIN_LIBRARY) or not os.path.exists(engine_file_path):
        print("not have FILE")
        exit(-1)
    ...
    # input_image_paths = ["1317039.jpeg"]
    ...
    while(cap.isOpened()):
        t0 = time.time()
        ret, image = cap.read()
        yolov5_wrapper.infer(image)
        dt = time.time() - t0
        accumulate_dt += dt
        count += 1
        if accumulate_dt > 1:
            print("FPS: ", count, int(1/dt))
            count = 0
            accumulate_dt = 0
